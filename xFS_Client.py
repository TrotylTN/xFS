...
you can enter \"help\" anytime to get it.\n")
  ...
  monitorThread = threading.Thread(target=monitorCMD, args=(sSock, localIP,
    localPort, trackingServer, trackingPort, logFile))
  monitorThread.start()
...
def monitorCMD(sSock, localIP, localPort, trackingServer, trackingPort, logFile):
  while 1:
    sentence = input()
    if sentence[:4].lower() == "exit":
      ...
      if not checkFileName(filename):
        print "Invalid file name, file name must longer than 0 and \
